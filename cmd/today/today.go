...
// scanMarkdownNotesFromToday scans the specified folder for markdown notes with today's date in YAML frontmatter
func scanMarkdownNotesFromToday(folderPath string) ([]string, error) {
	...
	err := filepath.WalkDir(folderPath, func(path string, d fs.DirEntry, err error) error {
		...
		if fileDate.Year() == today.Year() && fileDate.Month() == today.Month() && fileDate.Day() == today.Day() {
			todaysNotes = append(todaysNotes, path)
		}

		return nil
	})

	return todaysNotes, err
}
...
func Execute(folderPath string) error {
	...
	// Scan for today's notes
	notes, err := scanMarkdownNotesFromToday(folderPath)
	if err != nil {
